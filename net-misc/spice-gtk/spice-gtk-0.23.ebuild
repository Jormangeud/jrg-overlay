...
IUSE="+celt +controller dbus doc gstreamer gtk3 +introspection opus policykit pulseaudio
...
RDEPEND="${PYTHON_DEPS}
	controller? ( app-emulation/spice-protocol )
	pulseaudio? ( media-sound/pulseaudio[glib] )
	gstreamer? (
		media-libs/gstreamer:0.10
		media-libs/gst-plugins-base:0.10 )
	>=x11-libs/pixman-0.17.7
	celt? ( >=media-libs/celt-0.5.1.1:0.5.1 )
...
src_prepare() {
	...
	use static-libs || epatch "${FILESDIR}"/spice-gtk-0.23-skip-tests.patch
...
src_configure() {
	...
	myconf="
		$(use_enable celt celt051) \
		$(use_enable controller) \
