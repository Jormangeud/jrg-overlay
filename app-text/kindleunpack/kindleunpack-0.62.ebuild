EAPI=5
PYTHON_DEPEND="2"
SUPPORT_PYTHON_ABIS="1"
RESTRICT_PYTHON_ABIS="3.*"

inherit versionator distutils
MY_TAG="ed200966a44c"
MY_PN=${PN/-/_}
MY_PKG="Mekk-${MY_PN}-${MY_TAG}"
#MY_PV=$(replace_all_version_separators '-')

DESCRIPTION="Decompiles MobiPocket ebook files."
HOMEPAGE="http://wiki.mobileread.com/wiki/KindleUnpack"
SRC_URI="http://bitbucket.org/Mekk/${MY_PN}/get/${PV}.tar.bz2 -> ${P}.tar.bz2"
#SRC_URI="http://pypi.python.org/packages/source/m/mekk.calibre/mekk.calibre-${PV}.tar.gz"
RESTRICT="mirror"

LICENSE="BSD"
SLOT="0"
KEYWORDS="x86"
IUSE="chm djvu doc2rtf mso pdf"

DEPEND=">=dev-lang/python-2.6
		dev-python/lxml"
RDEPEND=">=app-text/calibre-0.7
		chm? ( app-text/archmage )
		djvu? ( app-text/djvu )
		doc2rtf? ( dev-python/ootools )
		mso? ( app-text/catdoc )
		pdf? ( app-text/poppler[utils] )
"
S="${WORKDIR}/${MY_PKG}"

src_install() {
	distutils_src_install
	dodoc README.txt
}
