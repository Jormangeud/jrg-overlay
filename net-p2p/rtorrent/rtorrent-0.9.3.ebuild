# Copyright 1999-2012 Gentoo Foundation
# Distributed under the terms of the GNU General Public License v2
# $Header: /var/cvsroot/gentoo-x86/net-p2p/rtorrent/rtorrent-0.9.2.ebuild,v 1.2 2012/09/10 02:43:00 ottxor Exp $

EAPI=4

inherit eutils autotools

DESCRIPTION="BitTorrent Client using libtorrent"
HOMEPAGE="http://libtorrent.rakshasa.no/"
SRC_URI="http://libtorrent.rakshasa.no/downloads/${P}.tar.gz"

LICENSE="GPL-2"
SLOT="0"
KEYWORDS="~amd64 ~arm ~hppa ~ia64 ~ppc ~ppc64 ~sparc ~x86 ~x86-fbsd ~amd64-linux ~x86-linux ~ppc-macos ~x64-macos ~x86-macos ~sparc-solaris ~x64-solaris"
IUSE="daemon debug ipv6 pyroscope test xmlrpc"

REQUIRED_USE="pyroscope? ( xmlrpc )"

COMMON_DEPEND="~net-libs/libtorrent-0.13.${PV##*.}
	>=dev-libs/libsigc++-2.2.2:2
	>=net-misc/curl-7.19.1
	sys-libs/ncurses
	xmlrpc? ( dev-libs/xmlrpc-c )"
RDEPEND="${COMMON_DEPEND}
	daemon? ( app-misc/screen )"
DEPEND="${COMMON_DEPEND}
	test? ( dev-util/cppunit )
	virtual/pkgconfig"

DOCS=( doc/rtorrent.rc )

src_prepare() {

	if ! use test; then
		sed -i -e '/AM_PATH_CPPUNIT/d' configure.ac
	fi

	if use pyroscope; then
		cp "${FILESDIR}"/{command_pyroscope.cc,ui_pyroscope.cc,ui_pyroscope.h} "${S}"/src
		epatch "${FILESDIR}"/ps-ui_pyroscope_0.8.8.patch
		epatch "${FILESDIR}"/pyroscope.patch
		epatch "${FILESDIR}"/ui_pyroscope.patch
		eautoreconf
	fi

	# bug #358271
	epatch "${FILESDIR}"/${PN}-0.9.1-ncurses.patch
}


src_configure() {
	# configure needs bash or script bombs out on some null shift, bug #291229
	CONFIG_SHELL=${BASH} econf \
		CXXFLAGS="${CXXFLAGS} -fno-strict-aliasing -Wno-conversion-null -std=c++11" \
		--disable-dependency-tracking \
		$(use_enable debug) \
		$(use_enable ipv6) \
		$(use_with xmlrpc xmlrpc-c)
#		CXXFLAGS="${CXXFLAGS} -fno-strict-aliasing -std=c++11 -lpthread" \
}

src_install() {
	default

	if use daemon; then
		newinitd "${FILESDIR}/rtorrentd.init" rtorrentd
		newconfd "${FILESDIR}/rtorrentd.conf" rtorrentd
	fi
}
