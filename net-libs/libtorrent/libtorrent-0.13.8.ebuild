...
IUSE="debug libressl pyroscope ssl test"
RESTRICT="!test? ( test )"
...
src_prepare() {
	...
	if use pyroscope; then
		...
		EPATCH_SOURCE="${FILESDIR}/ps/all" EPATCH_SUFFIX="patch" EPATCH_FORCE="yes" epatch || die
		EPATCH_SOURCE="${FILESDIR}/ps/${PV}" EPATCH_SUFFIX="patch" EPATCH_FORCE="yes" epatch || die
	fi
